...
from nuskell.include.peppercorn.reactions import ReactionPathway, get_auto_name
import nuskell.include.peppercorn.enumerator
...
def input_enum(filename):
	...
	enumerator = nuskell.include.peppercorn.enumerator.Enumerator(domains, strands, complexes)
	return enumerator
...
def enum_from_kernel(lines):
	...
	return nuskell.include.peppercorn.enumerator.Enumerator(domains, strands, complexes)
  ...
  for (line_counter, line) in enumerate(fin,start=1):
    ...
    # This is the beginning of a strand	
      ...
      for domain_name in filter(None,strand_dom_names.split()):
        ...
        elif domain_name not in domains:
          logging.error("Invalid domain name %s encountered in input line %d"
              % (domain_name, line_counter))
          logging.error(line)
          print domains
...
def load_json(filename):
	...
	enumerator = nuskell.include.peppercorn.enumerator.Enumerator(domains.values(), strands.values(), initial_complexes.values())
