...
def interpret_main(env, code):
  """ 
  Creates bindings for variables and functions defined in the body of the code.
...
def asgn_pattern_match(id, value):
    ...
    if type(id) == list:
        if type(value) != list or len(id) != len(value):
            raise RuntimeError("Pattern matching failed while assigning values to " + str(id) + ".")
        result = []
        ...
        raise RuntimeError("The left hand side of an assignment should be either an identifier or a list-tree consisting only of identifiers.")
...
def trim_id_list(l):
  ...
  if kwd == "idlist":
    ...
    raise RuntimeError(
        "The left hand side of an assignment should be either an identifier " + \
        "or a list-tree consisting only of identifiers.")
...
def eval_builtin_function(env, f, args):
  """ _sbad_ Evaluate builtin functions """
  if f == "tail":
    if type(args[0]) != list:
...
def flip(l, n):
  # flip is a function that works like a matrix transpose function for
  # lists of lists. Thus, if we compute flip(map(gmac,l),3), the above list
  # will turn into the form of [[S..,S..], [S..,S..], [S.., S..]].
  ...
  for i in range(len(l)):
    if len(l[i]) != n:
      raise RuntimeError("The argument of `flip' does not have a required form.")
    for j in range(n):
      res[j][i] = l[i][j]
  return res
...
def init_builtin_functions(env):
  ...
  env = env_create_binding(env, "complement", Function(["l"], "complement"))
