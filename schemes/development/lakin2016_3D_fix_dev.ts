...
#
#   Note: * The scheme implements Figure 8 from the above publication. Every 
#           reaction is expressed as bimolecular reaction:
#             A -> B + C := A + f -> B + C
#             A + B -> C := A + B -> f + C
#             A -> B := A + f -> f + B
#             A + B + C -> X + Y := A + B <=> f + i; i + C -> X + Y # not pathway equivalent
#             A + B -> X + Y + Z := A + B -> f + i; i -> X + Y
#             ...etc.
#
