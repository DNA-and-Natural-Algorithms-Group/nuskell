...
macro chen2D_fi(data, c) = "d t"|". ."
  where {
    ...
    pr = data[1];
    d = pr[c].x;
    t = if (c == (len(pr)-1)) then tr else (pr[c]).t };
...
module chen2D_JF_nm(react, prod, flux) = 
  ...
  | " (  ~  (  + ( (  +  )  )   )   ~   )   . " ] + 
  [ " tr r " | ". ." ] + # ifuels + # ifuels are additianl reverse fuels?
  [ " i + out + tr r + tq* r* tr* rout i* "
  | " ( +  ~  + (  ( +  .  )   )   ~   )  " ] + ofuels
    where {
      void = print(react, prod, flux);
      a = react[0].x;
      ta = react[0].t;
      ...
      tq = flux.tq;
      i = flux.di;
      ifuels = map2(chen2D_fi, [tr, react], range(len(react)));
...
function optimize(data, i) =
  if len(crn[i].reactants) > 0 and crn[i].reactants == r.reactants then 
    previous[i]
  else 0 
  where {
    crn = data[0];
