...
crn = ZeroOrMore(G(reaction | rev_reaction) | S(LineEnd())) + O(formal) + ZeroOrMore(S(LineEnd())) + O(constant) + ZeroOrMore(S(LineEnd()))
...
def parse_file(data):
    ...
    crn = document.parseFile(data, parseAll = True).asList()
    return _post_process(crn)

def parse_string(data):
    """Parses the given string and returns the result in the form of
       (crn, formal species, constant species)."""
    return _post_process(document.parseString(data).asList())
